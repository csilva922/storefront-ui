...
<template>
  ...
  <div v-if="content?.banner" class="relative">
    <div class="img">
      <NuxtImg v-if="$viewport.isGreaterOrEquals('tablet')" :src="content.banner.image?.url" :alt="content.banner.heading"
        class="w-full h-[500px] lg:h-[550px] 2xl:h-[620px] object-cover object-center" />
      <NuxtImg v-else :src="content.banner.image?.url" :alt="content.banner.heading"
        class="w-full max-h-[400px] object-cover object-center" />
    </div>

    <div
      class="text narrow-container w-full md:absolute top-[47%] left-[50%] -translate-half bg-[#F7F7F7] md:bg-transparent py-7">
      <div class="max-w-[470px] text-black md:text-white flex flex-col items-start gap-4">
        <h1 class="text-[28px] md:text-[36px] font-normal"> {{ content.banner.heading }}  </h1>
        <p class="text-[16px] md:text-[18px] mb-3">  {{ content.banner.subheading }} </p>
        <SfButton
          :tag="NuxtLink"
          :to="content.banner.buttonLink"
          type="button"
