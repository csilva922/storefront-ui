...
<template>
  ...
  <div v-if="content" class="relative">
    <div class="img">
      <NuxtImg v-if="$viewport.isGreaterOrEquals('tablet')" :src="content.image?.url" :alt="content.heading"
        class="w-full h-[500px] lg:h-[550px] 2xl:h-[620px] object-cover object-center" />
      <NuxtImg v-else :src="content.image?.url" :alt="content.heading"
    ...
    <div
      ...
      <div class="max-w-[470px] text-black md:text-white flex flex-col items-start gap-4">
        <h1 class="text-[28px] md:text-[36px] font-normal"> {{ content.heading }}  </h1>
        <p class="text-[16px] md:text-[18px] mb-3">  {{ content.subheading }} </p>
        <SfButton
          :tag="NuxtLink"
