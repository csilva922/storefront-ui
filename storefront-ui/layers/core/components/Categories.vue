...
const items = computed(() => {
  ...
  if (Array.isArray((props.content as any).items)) return (props.content as any).items;
  return [props.content];
});

const heading = computed(() => (props.content as any)?.heading);
</script>

<template>
  <div class="narrow-container pt-[40px] pb-[40px] md:pt-[80px] md:pb-[120px] space-y-6 md:space-y-10">
    <h3 v-if="heading" class="text-[24px] sm:text-[28px] font-normal mb-[30px] md:mb-[50px]">
      {{ heading }}
    </h3>
